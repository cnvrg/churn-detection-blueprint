...
y_test = args.y_test
var_smoothing = float(args.var_smoothing)
threshold = float(args.threshold)
id_column = args.id_column.split(',')

################## Defining Modelling Function and evaluation metrics function ###################
def modeling(alg, alg_name, var_smoothing, threshold):
    model = alg(var_smoothing=var_smoothing)
    print(X_train.shape)
    print('X_train')
...
log_model, y_pred_proba, y_pred, y_test, acc_score, pre_score, rec_score, f_score, y_cv_pred, scores, y_pred_train, acc_score_train, pre_score_train, rec_score_train, f_score_train = modeling(
    GaussianNB, 'Naive-Bayes', var_smoothing, threshold)

imps = permutation_importance(log_model, X_test, y_test)
var_imp_naive = pd.DataFrame({'variable':X_train.columns,'coefficient':imps.importances_mean}).sort_values('coefficient',ascending=False)

var_imp_naive = var_imp_naive.sort_values(by='coefficient', key=abs, ascending=False).reset_index(drop=True)

e = Experiment()

for i in range(min(var_imp_naive.shape[0],5)):
    e.log_param('metric-'+str(i)+var_imp_naive['variable'][i],var_imp_naive['coefficient'][i])

var_imp_naive.to_csv(cnvrg_workdir+'/important_metrics.csv',index=False)
...
eval_metrics = [['Naive-Bayes', mean(scores), acc_score, acc_score_train, pre_score,
